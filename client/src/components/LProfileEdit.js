    ...
    return (
      <div className="authForm">
        <form onSubmit={handleSubmit}>
          ...
          <p>
            <label htmlFor="picture">Change Your Profile Picture here:</label><br></br>
            <input
              type="picture"
              name="picture"
              value={picture}
        ...
        <button onClick={()=>setReveal(!reveal)}> Click Here to Edit Your Shelters</button>
        {reveal? <></>:
        <div>
          {currentUser.shelters.map((shelter) => <ShelterEdit key={shelter.id} shelter={shelter} />)}
      </div>}
      </div>
    )
}
